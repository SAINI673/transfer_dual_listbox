var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        $(transferId).find(tabItemName).on("click", function () {
            ...
            if (!$(this).is(".tab-active")) {
                ...
                $(transferId).find(selectedTotalNum).text("0 Item");
        ...
        $(deleteSelected).on("click", function () {
		 ...
		 $(listSearchId).on("keyup", function () {
			...
			var searchTerm = $(listSearchId).val().toLowerCase(); // Convert search term to lowercase
			...
			for (var j = 0; j < $(transferId).find(transferDoubleListLi).length; j++) {
				var listItemText = $(transferId).find(transferDoubleListLi).eq(j).text().toLowerCase(); // Convert list item text to lowercase

				if (!$(transferId).find(checkboxItem).eq(j).is(':checked') && listItemText.substr(0, searchTerm.length) == searchTerm) {
		...
		$(groupListSearchId).on("keyup", function () {
			...
			var searchTerm = $(groupListSearchId).val().toLowerCase(); // Convert search term to lowercase
			...
			for (var j = 0; j < $(transferId).find(transferDoubleGroupListLiUlLi).length; j++) {
				var listItemText = $(transferId).find(transferDoubleGroupListLiUlLi).eq(j).text().toLowerCase(); // Convert list item text to lowercase

				if (!$(transferId).find(groupCheckboxItem).eq(j).is(':checked') && listItemText.substr(0, searchTerm.length) == searchTerm) {
        ...
        $(selectedListSearchId).keyup(function () {
			...
			var searchTerm = $(selectedListSearchId).val().toLowerCase(); // Convert search term to lowercase
			...
			for (var i = 0; i < $(transferId).find(transferDoubleSelectedListLi).length; i++) {
				var listItemText = $(transferId).find(transferDoubleSelectedListLi).eq(i).text().toLowerCase(); // Convert list item text to lowercase

				if (listItemText.substr(0, searchTerm.length) == searchTerm) {
    ...
    function generateTransfer(inputId, currentTimeStr) {
        var htmlStr =
            ...
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + ' tab-active">Group</div>'
